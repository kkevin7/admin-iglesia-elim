...
const headCells = [
  ...
  { id: "empresa", numeric: true, disablePadding: false, label: "Empresa" },
...
const DataTableProveedores = ({ history, proveedores }) => {
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelecteds = proveedores.map(n => n.name);
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, proveedores.length - page * rowsPerPage);
    ...
    <Paper className={classes.paper}>
      ...
      >
        ...
        >
          ...
          />
          <TableBody>
            {stableSort(proveedores, getSorting(order, orderBy))
